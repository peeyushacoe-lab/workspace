...
export function ShareFileViewer({ file }: { file: SharedFile }) {
  ...
  return (
    <div className="flex min-h-screen flex-col bg-surface" style={{ fontFamily: "DM Sans, sans-serif" }}>
      {/* Top bar */}
      <header className="flex items-center gap-4 border-b border-border bg-surface px-6 py-3">
        <div className="flex items-center gap-3 flex-1 min-w-0">
          ...
          <div className="min-w-0">
            <h1 className="truncate text-base font-semibold text-foreground">{file.name}</h1>
            <p className="text-xs text-muted">{formatFileSize(file.size)} · {file.mimeType}</p>
          </div>
        </div>
      ...
      <main className="flex-1 flex items-center justify-center p-6 overflow-auto">
        {!file.downloadUrl ? (
          ...
          <div className="flex flex-col items-center gap-4">
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="bg-surface text-muted hover:bg-surface-sunken border border-border rounded-md px-3 py-1.5 text-sm font-medium transition-colors"
              >
                −
              </button>
          ...
          <div className="w-full max-w-lg rounded-2xl bg-surface p-10 shadow-md text-center border border-border">
            <File className="h-16 w-16 text-accent mx-auto mb-4" />
            <p className="text-sm font-medium text-foreground mb-6">{file.name}</p>
