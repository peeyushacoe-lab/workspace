        ...
        menus={[
          {
            id: "file", label: "File", entries: [
              { kind: "file", label: "Import Excel (.xlsx)\u2026", icon: Upload, accept: ".xlsx,.xls", onFile: e => importXLSX(e) },
              { kind: "sep" },
              { kind: "label", label: "Download" },
              { kind: "item", label: "Microsoft Excel (.xlsx)", icon: FileSpreadsheet, onSelect: () => void exportXLSX() },
              { kind: "item", label: "PDF (.pdf)", icon: Download, onSelect: () => exportSheetPdf() },
              { kind: "sep" },
              { kind: "item", label: "Print / Save as PDF", icon: Download, onSelect: () => printSheet() },
              { kind: "sep" },
              { kind: "item", label: "Version history", icon: History, onSelect: () => { setShowVersions(true); setShowComments(false); } },
              { kind: "item", label: "Templates\u2026", icon: Table, onSelect: () => setShowTemplates(true) },
            ],
          },
          {
            id: "edit", label: "Edit", entries: [
              { kind: "item", label: "Undo", icon: Undo2, hint: "\u2318Z", onSelect: () => undo() },
              { kind: "item", label: "Redo", icon: Redo2, hint: "\u2318\u21e7Z", onSelect: () => redo() },
              { kind: "sep" },
              { kind: "item", label: "Find and replace\u2026", icon: Search, onSelect: () => setShowFindReplace(true) },
              { kind: "item", label: "Remove duplicates\u2026", icon: CopyMinus, onSelect: () => setShowRemoveDupes(true) },
              { kind: "item", label: "Split text to columns\u2026", icon: SplitSquareHorizontal, onSelect: () => setShowSplitText(true) },
            ],
          },
          {
            id: "insert", label: "Insert", entries: [
              { kind: "item", label: "New sheet", icon: Plus, onSelect: () => addSheet() },
              { kind: "item", label: "Chart\u2026", icon: BarChart3, onSelect: () => setShowChart(true) },
              { kind: "item", label: "Sparkline", icon: BarChart3, onSelect: () => insertSparkline() },
              { kind: "item", label: "Note on cell\u2026", icon: MessageSquare, onSelect: () => setShowNote(true) },
              { kind: "item", label: "Pivot table\u2026", icon: Table, onSelect: () => setShowPivot(true) },
            ],
          },
          {
            id: "format", label: "Format", entries: [
              { kind: "item", label: "Conditional formatting\u2026", icon: Paintbrush, onSelect: () => setShowCF(true) },
              { kind: "item", label: "Data validation\u2026", icon: ListChecks, onSelect: () => setShowValidation(true) },
            ],
          },
          {
            id: "data", label: "Data", entries: [
              { kind: "item", label: "Sort and filter\u2026", icon: Filter, onSelect: () => setShowFilter(true) },
              { kind: "item", label: "Named ranges\u2026", icon: Tag, onSelect: () => setShowNames(true) },
              { kind: "sep" },
              { kind: "item", label: "Goal seek\u2026", icon: Target, onSelect: () => setShowGoalSeek(true) },
            ],
          },
          {
            id: "tools", label: "Tools", entries: [
              ...
              { kind: "sep" },
              { kind: "item", label: "Protect sheet", icon: Lock, onSelect: () => toggleSheetProtect() },
            ],
          },
        ]}
      />
