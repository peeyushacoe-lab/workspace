...
export function DocsView() {
  ...
  const editor = useEditor(
    {
      ...
      editorProps: {
        attributes: {
          class:
            "min-h-full outline-none text-sm text-[#202124] leading-relaxed prose prose-sm max-w-none prose-invert prose-headings:font-semibold prose-h1:text-2xl prose-h2:text-xl prose-h3:text-base prose-blockquote:border-l-4 prose-blockquote:border-[#1a56db]/30 prose-blockquote:pl-4 prose-blockquote:italic prose-blockquote:text-[#5f6368] prose-pre:bg-white prose-pre:text-[#202124] prose-pre:rounded-lg prose-pre:p-4 prose-pre:text-xs prose-pre:font-mono prose-a:text-[#1a56db] prose-a:underline",
        },
      },
      onUpdate: ({ editor: e }) => {
  ...
  return (
    <div className="flex h-[calc(100vh-3.5rem)] lg:h-screen bg-white overflow-hidden">
      ...
      <div className="bg-white border-r border-[#e8eaed] w-64 flex-shrink-0 flex flex-col">
        <div className="p-4 border-b border-[#e8eaed] flex items-center gap-2">
          <FileText className="w-5 h-5 text-[#1a56db]" />
          <span className="font-medium text-sm text-[#202124] truncate flex-1">Documents</span>
          <button
            onClick={handleNew}
            className="bg-[#1a56db] text-white hover:opacity-90 rounded-lg px-4 py-2 text-sm font-medium flex items-center gap-2 transition-opacity"
          >
        ...
        <div className="px-3 py-2 border-b border-[#e8eaed]">
          <div className="relative">
            ...
            <input
              ...
              className="bg-[#f1f3f4] border-transparent text-[#202124] placeholder-[#9aa3b8] rounded-lg pl-9 py-2 text-sm focus:ring-2 focus:ring-[#1a56db] focus:bg-[#f1f3f4] w-full outline-none"
            />
          </div>
        </div>
        ...
        <div className="flex-1 overflow-y-auto py-1">
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-12 text-[#5f6368]">
              ...
              <p className="text-xs text-[#5f6368]">{search ? "No documents found" : "No documents yet"}</p>
              {!search && (
                <button onClick={handleNew} className="mt-3 text-xs text-[#1a56db] hover:opacity-80 font-medium">
            ...
            docs.map((doc) => (
              <div
                ...
                className={`group mx-2 my-0.5 cursor-pointer ${
                  selectedId === doc.id
                    ? "bg-[#1a56db]/10 text-[#7dd8f5] rounded-lg px-3 py-2 font-medium text-sm"
              ...
              >
                <div className="flex items-start gap-2">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-[#202124] truncate">{doc.title || "Untitled"}</p>
                    <p className="text-xs text-[#5f6368] truncate mt-0.5">{docPreview(doc.content)}</p>
                  ...
                  <div className="opacity-0 group-hover:opacity-100 flex items-center gap-0.5 transition-opacity flex-shrink-0">
                    <button
                      ...
                      className="p-1 rounded text-[#5f6368] hover:text-amber-400 hover:bg-amber-400/10"
                    ...
                    <button
                      ...
                      className="p-1 rounded text-[#5f6368] hover:text-red-400 hover:bg-red-400/10"
                      title="Delete"
                    >
      ...
      {/* Editor area */}
      {!activeDoc ? (
        <div className="flex-1 flex flex-col items-center justify-center text-[#5f6368] bg-white">
          <FileText className="w-16 h-16 mb-4 opacity-20" />
          <p className="text-lg font-medium">No document selected</p>
          <p className="text-sm mt-1">Select a document or create a new one</p>
          <button
            onClick={handleNew}
            className="mt-5 bg-[#1a56db] text-white hover:opacity-90 rounded-lg px-4 py-2 text-sm font-medium w-full flex items-center gap-2 justify-center transition-opacity"
          >
        ...
        <div className="flex-1 flex flex-col bg-white overflow-hidden">
          {/* Doc header */}
          <div className="border-b border-[#e8eaed] px-8 py-4 flex items-center justify-between flex-shrink-0 bg-white">
            <input
              value={title}
              onChange={handleTitleChange}
              placeholder="Untitled Document"
              className="text-2xl font-semibold text-[#202124] border-none bg-transparent focus:ring-0 outline-none flex-1 px-6 py-4 placeholder-[#262b3a]"
            />
            <div className="flex items-center gap-3 ml-4 flex-shrink-0">
              {collaborators.length > 0 && (
                <div className="flex items-center gap-1 text-xs text-[#5f6368]">
                  <Users className="w-3 h-3 text-[#1a56db]" />
              ...
              {saveStatus === "unsaved" && (
                <span className="text-xs text-[#5f6368]">Unsaved changes</span>
              )}
            </div>
          </div>

          {/* Rich text toolbar */}
          <div className="border-b border-[#e8eaed] px-4 py-2 bg-white flex items-center gap-1 flex-wrap sticky top-0">
            {toolbar.map(({ label, action, active, icon: Icon }) => (
              <button
                key={label}
                onMouseDown={(e) => { e.preventDefault(); action(); }}
                title={label}
                className={`p-1.5 rounded transition-colors text-sm ${active ? "bg-[#1a56db]/10 text-[#1a56db]" : "text-[#5f6368] hover:bg-[#f1f3f4] hover:text-[#202124]"}`}
              >
            ...
            <button
              ...
              title="Ordered list"
              className="p-1.5 text-[#5f6368] hover:bg-[#f1f3f4] hover:text-[#202124] rounded transition-colors"
            >
            ...
            <select
              ...
              defaultValue=""
              className="text-xs text-[#5f6368] bg-transparent border-none outline-none cursor-pointer hover:text-[#202124] py-1"
            >
