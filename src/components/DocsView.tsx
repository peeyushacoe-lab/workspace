      ...
      {selectedId ? (
        <div className="flex flex-col flex-1 min-w-0 overflow-hidden">
          ...
          <div className="flex flex-wrap items-center gap-0.5 px-2 py-1 border-b border-[#e8eaed] bg-white z-10 overflow-x-auto" onClick={e => e.stopPropagation()}>
            ...
            <TB icon={<Table className="h-3.5 w-3.5" />} title="Insert table (3×3)" onClick={() => editor?.chain().focus().insertTable({ rows: 3, cols: 3, withHeaderRow: true }).run()} />
            <TB icon={<ImageIcon className="h-3.5 w-3.5" />} title="Insert image" onClick={() => { const u = prompt("Image URL:"); if (u) editor?.chain().focus().setImage?.({ src: u }).run(); }} />
