...
export function FormField({ label, hint, error, required, children, className }: FormFieldProps) {
  return (
    <div className={cn("flex flex-col gap-1.5", className)}>
      ...
      {children}
      {hint && !error && <p className="text-[11px] text-muted/60">{hint}</p>}
  ...
  ({ error, icon, className, ...props }, ref) => (
    <div className="relative">
      ...
      <input
        ref={ref}
        className={cn(
          "w-full rounded-lg border bg-surface-sunken px-3 py-2 text-sm text-foreground",
          "placeholder:text-muted/60",
          "focus:outline-none focus:ring-1 transition-colors",
          error
            ? "border-crit/50 focus:ring-crit/50"
            : "border-border/[0.07] focus:border-accent/50 focus:ring-accent/20",
          icon && "pl-9",
          className,
        )}
        {...props}
...
export const Textarea = forwardRef<HTMLTextAreaElement, TextareaProps>(
  ({ error, className, ...props }, ref) => (
    <textarea
      ...
      className={cn(
        "w-full rounded-lg border bg-surface-sunken px-3 py-2 text-sm text-foreground",
        "placeholder:text-muted/60 resize-y min-h-[80px]",
        "focus:outline-none focus:ring-1 transition-colors",
        error
          ? "border-crit/50 focus:ring-crit/50"
          : "border-border/[0.07] focus:border-accent/50 focus:ring-accent/20",
  ...
  ({ label, className, ...props }, ref) => (
    <label className="flex items-center gap-2 cursor-pointer select-none">
      <input
        ...
        type="checkbox"
        className={cn(
          "h-4 w-4 rounded border border-border/[0.2] bg-surface-sunken",
...
export function FormSection({
  ...
  return (
    <div className={cn("flex flex-col gap-4", className)}>
      {(title || description) && (
        <div className="border-b border-border/[0.07] pb-3">
