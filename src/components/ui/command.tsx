    ...
    >
      ...
      <div className="relative z-10 w-full max-w-lg rounded-xl border border-white/[0.1] bg-[#1b1f2e] shadow-[0_8px_32px_rgba(0,0,0,0.7)] overflow-hidden">
        ...
        <div className="flex items-center gap-3 px-4 py-3 border-b border-white/[0.07]">
          <Search className="h-4 w-4 text-[#bbc9cf] flex-shrink-0" />
          <input
            ref={inputRef}
            value={query}
            onChange={(e) => setQuery(e.target.value)}
            placeholder={placeholder}
            className="flex-1 bg-transparent text-sm text-[#dfe1f6] placeholder:text-[#bbc9cf]/60 focus:outline-none"
          />
          {query && (
          ...
          <kbd className="text-[10px] text-[#bbc9cf]/60 border border-white/[0.1] rounded px-1.5 py-0.5">
            ESC
          </kbd>
        </div>
        ...
        <div ref={listRef} className="max-h-80 overflow-y-auto py-1.5">
          {flat.length === 0 ? (
            <p className="px-4 py-8 text-center text-sm text-[#bbc9cf]">No results found</p>
          ) : (
            Object.entries(grouped).map(([group, groupItems]) => {
              const groupStart = flat.indexOf(groupItems[0]!);
              return (
                <div key={group}>
                  <p className="px-4 py-1.5 text-[10px] font-semibold uppercase tracking-wider text-[#bbc9cf]/50">
                    {group}
                  </p>
                  {groupItems.map((item) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="flex-1 min-w-0">
                          ...
                          {item.description && (
                            <span className="text-[11px] text-[#bbc9cf]/60 truncate block">
                              {item.description}
                            </span>
                          )}
                        </div>
                        {item.shortcut && (
                          <kbd className="text-[10px] text-[#bbc9cf]/50 border border-white/[0.1] rounded px-1.5 py-0.5 flex-shrink-0">
                            {item.shortcut}
                          </kbd>
                        )}
                        {idx === activeIndex && (
        ...
        <div className="px-4 py-2 border-t border-white/[0.07] flex items-center gap-4 text-[10px] text-[#bbc9cf]/50">
