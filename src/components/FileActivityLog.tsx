...
export function FileActivityLog({ fileId }: Props) {
  ...
  if (entries.length === 0) {
    return (
      <div className="flex flex-col items-center gap-2 py-10 text-[#bbc9cf]">
  ...
  return (
    <div className="py-2">
      {entries.map((entry) => {
        ...
        return (
          <div key={entry.id} className="border-b border-[rgba(0,255,255,0.08)] px-4 py-3 hover:bg-[#262939] transition-colors flex items-start gap-3">
            <div className="w-8 h-8 rounded-lg bg-[#262939] flex items-center justify-center text-[#bbc9cf] flex-shrink-0 mt-0.5">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-sm text-[#dfe1f6]">
                ...
                {" "}
                <span className="text-[#bbc9cf]">{getActionLabel(entry.action)}</span>
              </p>
              {entry.actorEmail && (
                <p className="text-xs text-[#bbc9cf] truncate">{entry.actorEmail}</p>
              )}
