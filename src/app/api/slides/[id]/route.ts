...
async function getAccess(docId: string, userId: string, ownerId: string, userRole: string) {
  ...
  const r = await redis.hget(`doc:share:pres:${docId}`, userId).catch(() => null);
...
export async function GET(_req: Request, { params }: Params) {
  ...
  const pres = await prisma.note.findFirst({ where: { id, color: SLIDE_MARKER } });
  ...
  return NextResponse.json({ ...pres, access });
}
...
export async function DELETE(_req: Request, { params }: Params) {
  ...
  const pres = await prisma.note.findFirst({ where: { id, color: SLIDE_MARKER } });
  if (!pres) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (pres.userId !== user.id && !["ADMIN","CEO","CISO"].includes(user.role))
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  await prisma.note.delete({ where: { id } });
  await redis.del(`doc:share:pres:${id}`).catch(() => {});
