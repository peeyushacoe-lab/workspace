...
export async function GET(request: Request, { params }: Params) {
  ...
  const session = await prisma.docSession.upsert({
    where: { id: `sheet:${documentId}:${user.id}` },
    create: {
      id: `sheet:${documentId}:${user.id}`,
  ...
  if (session) {
    await redis.publish(`sheet:${documentId}`, JSON.stringify({
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const activeSessions = await prisma.docSession.findMany({
        where: { documentId: `sheet:${documentId}`, isActive: true, userId: { not: user.id } },
      });
      ...
      try {
        ...
        await new Promise<void>((resolve, reject) => {
          sub!.subscribe(`sheet:${documentId}`, (err) => err ? reject(err) : resolve());
        });
      ...
      await prisma.docSession.updateMany({
        where: { documentId: `sheet:${documentId}`, userId: user.id },
        data: { isActive: false },
      }).catch(() => {});
      await redis.publish(`sheet:${documentId}`, JSON.stringify({
