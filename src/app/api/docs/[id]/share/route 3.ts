...
export async function GET(_req: Request, { params }: Params) {
  ...
  const doc = await prisma.note.findFirst({ where: { id, color: DOC_MARKER } });
  if (!doc) return NextResponse.json({ error: "Not found" }, { status: 404 });
  if (doc.userId !== user.id && !["ADMIN", "CEO", "CISO"].includes(user.role))
    return NextResponse.json({ error: "Forbidden" }, { status: 403 });

  const raw = await redis.hgetall(docShareKey(id)) as Record<string, string> | null;
...
export async function POST(request: Request, { params }: Params) {
  ...
  await redis.hset(docShareKey(id), { [targetUser.id]: role });
...
export async function DELETE(request: Request, { params }: Params) {
  ...
  if (userId) await redis.hdel(docShareKey(id), userId);
