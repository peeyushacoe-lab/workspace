  ...
  return (
    <div className="min-h-screen bg-[#0f1321] text-[#dfe1f6]">
      ...
      <div className="px-6 pb-8 max-w-3xl space-y-4">
        <div className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl p-5 space-y-3">
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Meeting title (optional)"
            className="w-full bg-[#0c0e1a] border border-[rgba(0,255,255,0.1)] rounded-lg px-3 py-2 text-sm text-[#dfe1f6] placeholder-[#5c6b72] focus:outline-none focus:border-[#00d2ff]/40"
          />
          <textarea
            value={transcript}
            ...
            rows={10}
            className="w-full bg-[#0c0e1a] border border-[rgba(0,255,255,0.1)] rounded-lg px-3 py-2 text-sm text-[#dfe1f6] placeholder-[#5c6b72] focus:outline-none focus:border-[#00d2ff]/40 resize-none font-mono"
          />
          <div className="flex items-center justify-between">
            <span className="text-xs text-[#5c6b72]">{transcript.length.toLocaleString()} / 80,000 chars</span>
            <button
              onClick={analyze}
        ...
        {result && (
          <div className="space-y-3">
            {/* Meta row */}
            <div className="flex flex-wrap items-center gap-3 text-xs bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl px-4 py-3">
              <span className={`font-semibold capitalize ${SENTIMENT_COLOR[result.sentiment] ?? "text-[#bbc9cf]"}`}>
                {result.sentiment} sentiment
              </span>
              {result.attendees.length > 0 && (
                <span className="text-[#5c6b72] flex items-center gap-1">
                  <Users className="w-3 h-3" />{result.attendees.join(", ")}
                </span>
              )}
              <div className="flex gap-1.5 flex-wrap">
                {result.tags.map((t) => (
                  <span key={t} className="bg-[#00d2ff]/10 text-[#a5e7ff] px-2 py-0.5 rounded-full">{t}</span>
                ))}
              </div>
              <div className="flex-1" />
              <button onClick={exportMd} className="flex items-center gap-1 text-[#5c6b72] hover:text-[#00d2ff] transition-colors">
                <Download className="w-3.5 h-3.5" /> Export .md
              </button>
              <button
                onClick={() => navigator.clipboard.writeText(JSON.stringify(result, null, 2)).then(() => toast.success("Copied")).catch(() => {})}
                className="flex items-center gap-1 text-[#5c6b72] hover:text-[#00d2ff] transition-colors"
              >
            ...
            {[
              { key: "summary", label: "Summary", icon: FileText, content: (
                <p className="text-sm text-[#bbc9cf] leading-relaxed">{result.summary}</p>
              )},
              { key: "actions", label: `Action Items (${result.actionItems.length})`, icon: CheckSquare, content: (
                <ul className="space-y-2">
                  {result.actionItems.length === 0 ? (
                    <li className="text-xs text-[#5c6b72]">None identified</li>
                  ) : result.actionItems.map((a, i) => (
                    <li key={i} className="flex items-start gap-2 text-sm">
                      <span className="w-4 h-4 border border-[rgba(0,255,255,0.2)] rounded flex-shrink-0 mt-0.5" />
                      <span>
                        <strong className="text-[#00d2ff]">{a.owner}:</strong>{" "}
                        {a.task}
                        {a.due && <span className="text-[#5c6b72] ml-1">· {a.due}</span>}
                      </span>
                    </li>
                  ))}
                </ul>
              ...
              { key: "decisions", label: `Decisions (${result.decisions.length})`, icon: CheckSquare, content: (
                <ul className="space-y-1.5">
                  {result.decisions.length === 0 ? (
                    <li className="text-xs text-[#5c6b72]">None recorded</li>
                  ) : result.decisions.map((d, i) => (
                    <li key={i} className="text-sm text-[#bbc9cf] flex items-start gap-2">
              ...
              <div key={key} className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl overflow-hidden">
                <button
                  onClick={() => toggle(key)}
                ...
                >
                  ...
                  {label}
                  <div className="flex-1" />
                  {open[key] ? <ChevronDown className="w-4 h-4 text-[#5c6b72]" /> : <ChevronRight className="w-4 h-4 text-[#5c6b72]" />}
                </button>
