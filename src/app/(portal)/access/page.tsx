  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        disabled={disabled}
        className="flex items-center gap-1 text-xs text-[#9aa0a6] hover:text-[#1a56db] border border-dashed border-[rgba(255,255,255,0.11)] hover:border-[#1a56db]/40 px-2 py-1 rounded-full transition-colors disabled:opacity-40"
      >
        + Grant access
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 z-20 w-48 bg-white border border-[#e8eaed] rounded-xl shadow-xl overflow-hidden">
            {available.map((role) => (
              <button
                key={role}
                onClick={() => { onGrant(role); setOpen(false); }}
                className="w-full text-left px-3 py-2 text-sm text-[#5f6368] hover:bg-[#f1f3f4] hover:text-[#202124] transition-colors"
              >
                {role}
              </button>
            ))}
  ...
  return (
    <div className="min-h-screen bg-white text-[#202124]">
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
        {/* Info banner */}
        <div className="flex items-start gap-3 bg-[#1a56db]/5 border border-[#1a56db]/15 rounded-xl px-4 py-3">
          ...
          <p className="text-xs text-[#5f6368] leading-relaxed">
            ...
            Click a role badge to revoke it. Use the <strong className="text-[#202124]">+ Grant access</strong> button to add new access categories.
          </p>
        </div>

        {/* Search */}
        <div className="flex items-center gap-2 bg-white border border-[#e8eaed] rounded-xl px-4 py-2.5">
          ...
          />
          {query && (
            <button onClick={() => setQuery("")} className="text-[#9aa0a6] hover:text-[#5f6368]">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        ...
        <div className="bg-white border border-[#e8eaed] rounded-xl p-4">
          <p className="text-xs font-semibold text-[#9aa0a6] mb-3">Available Access Categories</p>
          <div className="flex flex-wrap gap-2">
            {GRANTABLE_ACCESS.map((r) => (
              <span key={r} className="text-xs text-[#5f6368] bg-[#f1f3f4] border border-[#e8eaed] px-2.5 py-1 rounded-full">
                {r}
              </span>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <p className="text-center text-[#9aa0a6] text-sm py-8">No users found</p>
        ) : (
          <div className="space-y-2">
            {filtered.map((user) => (
              <div
                key={user.id}
                className="bg-white border border-[#e8eaed] rounded-xl px-5 py-4 flex items-start gap-4 hover:border-[rgba(255,255,255,0.11)] transition-colors"
              >
                <Avatar name={user.fullName} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="font-semibold text-sm text-[#202124]">{user.fullName}</span>
                    <span className="text-[10px] text-[#9aa0a6] bg-[#f1f3f4] border border-[#e8eaed] px-1.5 py-0.5 rounded-full">
                      {roleLabels[user.role] ?? user.role}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 text-[#9aa0a6] text-xs mb-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {user.grantedRoles.length > 0 ? (
                      ...
                      <span className="text-xs text-[#9aa0a6] italic">No additional access granted</span>
                    )}
                    <GrantDropdown
                    ...
                    />
                    {saving === user.id && <Loader2 className="w-3.5 h-3.5 animate-spin text-[#5f6368]" />}
                  </div>
                </div>
              </div>
            ))}
