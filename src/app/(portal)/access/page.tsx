...
function Avatar({ name, size = "md" }: { name: string; size?: "sm" | "md" }) {
  ...
  return (
    <div className={`${sz} rounded-full bg-gradient-to-br from-accent/15 to-accent/10 border border-accent/[0.2] flex items-center justify-center font-semibold text-accent flex-shrink-0`}>
      {initials}
    </div>
  );
}
...
function RoleBadge({ role, onRevoke, disabled }: { role: string; onRevoke: () => void; disabled: boolean }) {
  return (
    <button
      ...
      className="group flex items-center gap-1 bg-accent/10 text-accent border border-accent/20 px-2.5 py-1 rounded-full text-xs font-semibold hover:bg-crit/10 hover:text-crit hover:border-crit/30 transition-colors disabled:opacity-40 disabled:cursor-not-allowed"
    >
      {role}
  ...
  return (
    <div className="relative">
      <button
        onClick={() => setOpen((v) => !v)}
        disabled={disabled}
        className="flex items-center gap-1 text-xs text-subtle hover:text-accent border border-dashed border-border hover:border-accent/40 px-2 py-1 rounded-full transition-colors disabled:opacity-40"
      >
        + Grant access
      ...
      {open && (
        <>
          ...
          <div className="absolute left-0 top-full mt-1 z-20 w-48 bg-surface border border-border rounded-xl shadow-xl overflow-hidden">
            {available.map((role) => (
              <button
                key={role}
                onClick={() => { onGrant(role); setOpen(false); }}
                className="w-full text-left px-3 py-2 text-sm text-muted hover:bg-surface-sunken hover:text-foreground transition-colors"
              >
                {role}
              </button>
            ))}
      ...
      <div className="max-w-5xl mx-auto px-6 py-6 space-y-6">
        ...
        <div className="flex items-start gap-3 bg-accent/5 border border-accent/15 rounded-xl px-4 py-3">
          ...
          <p className="text-xs text-muted leading-relaxed">
            ...
            Click a role badge to revoke it. Use the <strong className="text-foreground">+ Grant access</strong> button to add new access categories.
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 bg-surface border border-border rounded-xl px-4 py-2.5">
          ...
          />
          {query && (
            <button onClick={() => setQuery("")} className="text-subtle hover:text-muted">
              <X className="w-4 h-4" />
            </button>
          )}
        </div>
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <p className="text-xs font-semibold text-subtle mb-3">Available Access Categories</p>
          <div className="flex flex-wrap gap-2">
            {GRANTABLE_ACCESS.map((r) => (
              <span key={r} className="text-xs text-muted bg-surface-sunken border border-border px-2.5 py-1 rounded-full">
                {r}
              </span>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <p className="text-center text-subtle text-sm py-8">No users found</p>
        ) : (
          <div className="space-y-2">
            {filtered.map((user) => (
              <div
                key={user.id}
                className="bg-surface border border-border rounded-xl px-5 py-4 flex items-start gap-4 hover:border-border transition-colors"
              >
                <Avatar name={user.fullName} />
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    <span className="font-semibold text-sm text-foreground">{user.fullName}</span>
                    <span className="text-[10px] text-subtle bg-surface-sunken border border-border px-1.5 py-0.5 rounded-full">
                      {roleLabels[user.role] ?? user.role}
                    </span>
                  </div>
                  <div className="flex items-center gap-1 text-subtle text-xs mb-3">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    {user.grantedRoles.length > 0 ? (
                      ...
                      <span className="text-xs text-subtle italic">No additional access granted</span>
                    )}
                    <GrantDropdown
                    ...
                    />
                    {saving === user.id && <Loader2 className="w-3.5 h-3.5 animate-spin text-muted" />}
                  </div>
                </div>
              </div>
            ))}
