    ...
    <div className="max-w-lg mx-auto mt-16">
      <div className="bg-surface border border-border rounded-2xl p-8 text-center space-y-6">
        <div className="w-16 h-16 rounded-2xl bg-foreground flex items-center justify-center mx-auto">
          <GitBranch className="w-8 h-8 text-surface" />
        </div>
        <div>
        ...
        <div className="text-left space-y-3">
          ...
          <p className="text-xs text-subtle">
            ...
            . Needs <code className="bg-surface-sunken px-1 rounded text-[11px]">repo</code> and{" "}
            <code className="bg-surface-sunken px-1 rounded text-[11px]">read:user</code> scopes.
          </p>
        </div>
        <button
          ...
          className="w-full flex items-center justify-center gap-2 px-4 py-2.5 text-sm font-semibold
                     bg-hover text-foreground rounded-lg hover:bg-surface-sunken disabled:opacity-50 transition-colors">
...
function RepoCard({ repo }: { repo: GHRepo }) {
  const langColor = repo.language ? (LANG_COLORS[repo.language] ?? "#6b6a65") : "#6b6a65";
  return (
    <a href={repo.html_url} target="_blank" rel="noopener noreferrer"
      className="flex flex-col gap-2 p-4 bg-surface border border-border rounded-xl hover:border-accent/30 hover:shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {repo.private ? <Lock className="w-3.5 h-3.5 text-subtle shrink-0" /> : <Globe className="w-3.5 h-3.5 text-subtle shrink-0" />}
          <span className="text-sm font-semibold text-accent truncate group-hover:underline">{repo.name}</span>
        </div>
        <ExternalLink className="w-3.5 h-3.5 text-subtle shrink-0 mt-0.5" />
      </div>
      {repo.description && (
        <p className="text-xs text-muted leading-relaxed line-clamp-2">{repo.description}</p>
      )}
      <div className="flex items-center gap-3 text-xs text-subtle mt-auto">
        {repo.language && (
  ...
  return (
    <a href={item.html_url} target="_blank" rel="noopener noreferrer"
      ...
      <div className="mt-0.5 shrink-0">
        {isPR
          ? <GitPullRequest className="w-4 h-4 text-violet" />
          : <Circle className="w-4 h-4 text-ok" />}
      </div>
      ...
      <div className="px-6 pb-12 max-w-6xl space-y-6">
        ...
        {user && (
          <div className="flex items-center gap-4 p-4 bg-surface border border-border rounded-xl">
            ...
            <img src={user.avatar_url} alt={user.login} className="w-12 h-12 rounded-full border border-border" />
            <div>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-foreground">{user.name || user.login}</span>
            ...
            <div className="ml-auto flex items-center gap-6 text-sm">
              <div className="text-center">
                <div className="font-semibold text-foreground">{repos.length}</div>
                <div className="text-xs text-subtle">Repos</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-foreground">{prs.length}</div>
                <div className="text-xs text-subtle">Open PRs</div>
              </div>
              <div className="text-center">
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              {tab.count !== undefined && (
                <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${
                  activeTab === tab.id ? "bg-accent-soft text-accent" : "bg-surface-sunken text-subtle"
                }`}>{tab.count}</span>
              )}
            </button>
          ))}
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-foreground mb-3">Recent Repositories</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h3 className="text-sm font-semibold text-foreground mb-2">Open Pull Requests</h3>
                <div className="bg-surface border border-border rounded-xl divide-y divide-border overflow-hidden">
                  {prs.length === 0
                    ? <div className="py-8 text-center text-sm text-subtle">No open pull requests</div>
                    : prs.slice(0, 5).map(pr => <IssueRow key={pr.id} item={pr} isPR />)}
                </div>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-foreground mb-2">Assigned Issues</h3>
                <div className="bg-surface border border-border rounded-xl divide-y divide-border overflow-hidden">
                  {issues.length === 0
                    ? <div className="py-8 text-center text-sm text-subtle">No assigned issues</div>
        ...
        {activeTab === "repos" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {repos.length === 0
              ? <div className="col-span-3 py-16 text-center text-sm text-subtle">No repositories found</div>
        ...
        {activeTab === "prs" && (
          <div className="bg-surface border border-border rounded-xl divide-y divide-border overflow-hidden">
            {prs.length === 0
              ? <div className="py-16 text-center text-sm text-subtle">No open pull requests</div>
        ...
        {activeTab === "issues" && (
          <div className="bg-surface border border-border rounded-xl divide-y divide-border overflow-hidden">
            {issues.length === 0
              ? <div className="py-16 text-center text-sm text-subtle">No assigned issues</div>
