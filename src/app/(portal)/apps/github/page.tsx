  ...
  return (
    <div className="max-w-lg mx-auto mt-16">
      <div className="bg-white border border-[#e8eaed] rounded-2xl p-8 text-center space-y-6">
        ...
        <div>
          <h2 className="text-lg font-semibold text-[#202124]">Connect GitHub</h2>
          <p className="text-sm text-[#5f6368] mt-1">
            Link your GitHub account to view repos, pull requests, and issues directly in Nexus.
          </p>
        </div>
        <div className="text-left space-y-3">
          <label className="text-xs font-medium text-[#5f6368] flex items-center gap-1.5">
          ...
          <input
            ...
            className="w-full px-3 py-2.5 bg-[#f1f3f4] border border-[#d0d5dd] rounded-lg text-sm font-mono
                       placeholder:text-[#80868b] focus:outline-none focus:border-[#1a56db]/60 focus:ring-2 focus:ring-[#1a56db]/20"
          />
          <p className="text-xs text-[#80868b]">
            ...
            . Needs <code className="bg-[#f1f3f4] px-1 rounded text-[11px]">repo</code> and{" "}
            <code className="bg-[#f1f3f4] px-1 rounded text-[11px]">read:user</code> scopes.
          </p>
        </div>
        <button
...
function RepoCard({ repo }: { repo: GHRepo }) {
  const langColor = repo.language ? (LANG_COLORS[repo.language] ?? "#5f6368") : "#5f6368";
  return (
    <a href={repo.html_url} target="_blank" rel="noopener noreferrer"
      className="flex flex-col gap-2 p-4 bg-white border border-[#e8eaed] rounded-xl hover:border-[#1a56db]/30 hover:shadow-sm transition-all group">
      <div className="flex items-start justify-between gap-2">
        <div className="flex items-center gap-2 min-w-0">
          {repo.private ? <Lock className="w-3.5 h-3.5 text-[#80868b] shrink-0" /> : <Globe className="w-3.5 h-3.5 text-[#80868b] shrink-0" />}
          <span className="text-sm font-semibold text-[#1a56db] truncate group-hover:underline">{repo.name}</span>
        </div>
        <ExternalLink className="w-3.5 h-3.5 text-[#bdc1c6] shrink-0 mt-0.5" />
      </div>
      {repo.description && (
        <p className="text-xs text-[#5f6368] leading-relaxed line-clamp-2">{repo.description}</p>
      )}
      <div className="flex items-center gap-3 text-xs text-[#80868b] mt-auto">
        {repo.language && (
    ...
    <a href={item.html_url} target="_blank" rel="noopener noreferrer"
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="text-sm font-medium text-[#202124] group-hover:text-[#1a56db] transition-colors line-clamp-1">
            {item.title}
          </span>
          {item.labels.map(l => (
        ...
        <div className="flex items-center gap-2 text-xs text-[#80868b] mt-0.5">
      ...
      <div className="px-6 pb-12 max-w-6xl space-y-6">
        ...
        {user && (
          <div className="flex items-center gap-4 p-4 bg-[#f8f9fa] border border-[#e8eaed] rounded-xl">
            ...
            <img src={user.avatar_url} alt={user.login} className="w-12 h-12 rounded-full border border-[#e8eaed]" />
            <div>
              <div className="flex items-center gap-2">
                <span className="font-semibold text-[#202124]">{user.name || user.login}</span>
                <CheckCircle2 className="w-4 h-4 text-emerald-500" />
                <span className="text-xs text-emerald-600 font-medium">Connected</span>
              </div>
              <span className="text-sm text-[#5f6368]">@{user.login}</span>
            </div>
            <div className="ml-auto flex items-center gap-6 text-sm">
              <div className="text-center">
                <div className="font-semibold text-[#202124]">{repos.length}</div>
                <div className="text-xs text-[#80868b]">Repos</div>
              </div>
              <div className="text-center">
                <div className="font-semibold text-[#202124]">{prs.length}</div>
                <div className="text-xs text-[#80868b]">Open PRs</div>
              </div>
              <div className="text-center">
            ...
            <button key={tab.id} onClick={() => setActiveTab(tab.id)}
              ...
              {tab.count !== undefined && (
                <span className={`text-[11px] px-1.5 py-0.5 rounded-full font-semibold ${
                  activeTab === tab.id ? "bg-[#e8f0fe] text-[#1a56db]" : "bg-[#f1f3f4] text-[#80868b]"
                }`}>{tab.count}</span>
              )}
            </button>
          ))}
        ...
        {activeTab === "overview" && (
          <div className="space-y-6">
            <div>
              <h3 className="text-sm font-semibold text-[#202124] mb-3">Recent Repositories</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              <div>
                <h3 className="text-sm font-semibold text-[#202124] mb-2">Open Pull Requests</h3>
                <div className="bg-white border border-[#e8eaed] rounded-xl divide-y divide-[#e8eaed] overflow-hidden">
                  {prs.length === 0
                    ? <div className="py-8 text-center text-sm text-[#80868b]">No open pull requests</div>
                    : prs.slice(0, 5).map(pr => <IssueRow key={pr.id} item={pr} isPR />)}
                </div>
              </div>
              <div>
                <h3 className="text-sm font-semibold text-[#202124] mb-2">Assigned Issues</h3>
                <div className="bg-white border border-[#e8eaed] rounded-xl divide-y divide-[#e8eaed] overflow-hidden">
                  {issues.length === 0
                    ? <div className="py-8 text-center text-sm text-[#80868b]">No assigned issues</div>
        ...
        {activeTab === "repos" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
            {repos.length === 0
              ? <div className="col-span-3 py-16 text-center text-sm text-[#80868b]">No repositories found</div>
        ...
        {activeTab === "prs" && (
          <div className="bg-white border border-[#e8eaed] rounded-xl divide-y divide-[#e8eaed] overflow-hidden">
            {prs.length === 0
              ? <div className="py-16 text-center text-sm text-[#80868b]">No open pull requests</div>
        ...
        {activeTab === "issues" && (
          <div className="bg-white border border-[#e8eaed] rounded-xl divide-y divide-[#e8eaed] overflow-hidden">
            {issues.length === 0
              ? <div className="py-16 text-center text-sm text-[#80868b]">No assigned issues</div>
