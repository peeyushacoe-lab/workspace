...
function Stat({ label, value }: { label: string; value: number | string }) {
  return (
    <div className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl p-4">
      <p className="text-[10px] text-[#5c6b72] uppercase tracking-wider mb-1">{label}</p>
      <p className="text-2xl font-bold text-[#dfe1f6]">{typeof value === "number" ? value.toLocaleString() : value}</p>
    </div>
  );
}
    ...
    <div className="min-h-screen bg-[#0f1321] text-[#dfe1f6] p-6">
      <div className="max-w-6xl mx-auto space-y-6">
        <div className="flex items-center justify-between">
          <div>
            <p className="text-[10px] text-[#00d2ff] uppercase tracking-widest mb-1">Observability · Phase 28</p>
            <h1 className="text-2xl font-bold">System Health</h1>
          </div>
          <button onClick={refresh} className="flex items-center gap-2 px-4 py-2 rounded-lg bg-[#262939] text-[#bbc9cf] text-sm hover:bg-[#2e3347]">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          {[
            ...
            <div key={label} className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl p-4 flex items-center gap-3">
              <Icon className="w-5 h-5 text-[#5c6b72]" />
        ...
        {health?.queues && (
          <div className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl overflow-hidden">
            <div className="flex items-center gap-2 px-4 py-3 border-b border-[rgba(0,255,255,0.08)]">
            ...
            <table className="w-full text-xs">
              ...
              <tbody>
                {Object.entries(health.queues).map(([name, q]) => (
                  <tr key={name} className="border-b border-[rgba(0,255,255,0.03)] hover:bg-[#262939]/30">
                    <td className="px-4 py-2 font-mono text-[#bbc9cf]">{name}</td>
                    <td className="px-4 py-2 text-right text-[#bbc9cf]">{q?.waiting ?? "—"}</td>
                    <td className="px-4 py-2 text-right text-[#00d2ff]">{q?.active ?? "—"}</td>
                    <td className={`px-4 py-2 text-right ${q && q.failed > 0 ? "text-[#ff4d6d]" : "text-[#bbc9cf]"}`}>{q?.failed ?? "—"}</td>
                    <td className="px-4 py-2 text-right text-[#bbc9cf]">{q?.delayed ?? "—"}</td>
