...
import {
  ...
  Download, FileText, Loader2, Network, Plus, RefreshCw, Save, ShieldCheck, Trash2, Upload, Users, X,
} from "lucide-react";
...
const STATUS_STYLE: Record<string, string> = {
  ...
  CANCELLED: "bg-surface-sunken text-muted border-border",
};
...
function StatCard({ label, value, accent }: { label: string; value: number | string; accent?: boolean }) {
  return (
    <div className="bg-surface border border-border rounded-xl p-4">
      ...
      <p className={`text-2xl font-semibold tracking-tight mt-1 ${accent ? "text-accent" : "text-foreground"}`}>{value}</p>
    </div>
  );
}
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        <div className="bg-surface border border-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">On leave today</h3>
          {data.onLeaveToday.length === 0 ? (
            <p className="text-sm text-muted">Everyone is in — no approved leave covers today.</p>
          ) : (
              ...
              {data.onLeaveToday.map(l => (
                <div key={l.id} className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    <p className="text-sm text-foreground truncate">{l.user.fullName}</p>
                    <p className="text-xs text-muted">{l.type.toLowerCase()} · until {fmt(l.endDate)}</p>
                  </div>
                </div>
              ))}
            </div>
          )}
          <h3 className="text-sm font-semibold text-foreground mt-5 mb-2">Upcoming holidays</h3>
          {data.upcomingHolidays.length === 0 ? (
            <p className="text-sm text-muted">None in the next 30 days.</p>
          ) : (
        ...
        <div className="bg-surface border border-border rounded-xl p-4">
          <h3 className="text-sm font-semibold text-foreground mb-3">Headcount by department</h3>
          {deptEntries.length === 0 ? (
            <p className="text-sm text-muted">No department data yet. Fill in the People tab.</p>
          ) : (
              ...
              {deptEntries.map(([dept, count]) => (
                <div key={dept}>
                  ...
                  <div className="h-1.5 rounded-full bg-surface-sunken overflow-hidden">
                    <div className="h-full rounded-full bg-accent" style={{ width: `${(count / maxDept) * 100}%` }} />
                  </div>
                </div>
              ))}
            </div>
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <p className="text-sm text-muted">Staff records · <span className="text-foreground font-medium">{rows.length}</span></p>
        <button onClick={backfill} disabled={backfilling || missingCount === 0}
          className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg border border-border text-muted hover:bg-surface-sunken disabled:opacity-50 transition-colors">
      ...
      {loading ? (
        <div className="flex items-center justify-center py-16"><Loader2 className="w-7 h-7 text-accent animate-spin" /></div>
      ) : rows.length === 0 ? (
        <div className="flex flex-col items-center justify-center py-16 text-center">
          ...
          <p className="text-sm text-muted mt-1 max-w-xs">No non-intern accounts exist yet.</p>
        </div>
      ) : (
              ...
              <div key={r.id} className="bg-surface border border-border rounded-xl p-4">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-[11px] text-muted truncate">{r.email} · {r.role}</p>
                  </div>
                  <button onClick={() => setExpanded(expanded === r.id ? null : r.id)}
                    className="ml-auto flex items-center gap-1 px-2.5 py-1.5 text-xs font-medium rounded-lg text-muted hover:text-foreground hover:bg-surface-sunken transition-colors">
                  ...
                  <button onClick={() => saveRow(r.id)} disabled={savingId === r.id}
                    className="flex items-center gap-1.5 px-3 py-1.5 bg-accent text-accent-foreground text-xs font-semibold rounded-lg hover:bg-accent-hover disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="bg-surface border border-border rounded-xl p-4">
      <button onClick={() => setOpen(v => !v)} className="w-full flex items-center gap-2 text-left">
        ...
        <p className="text-sm font-semibold text-foreground">Letter signatories</p>
        <span className="text-xs text-muted">— who signs onboarding/exit letters &amp; NOCs</span>
        {open ? <ChevronDown className="w-4 h-4 text-muted ml-auto" /> : <ChevronRight className="w-4 h-4 text-muted ml-auto" />}
      </button>
      ...
      {open && (
        <div className="mt-3 pt-3 border-t border-border space-y-2">
          ...
          {list === null ? (
            ...
            list.map(s => (
              <div key={s.id} className="flex items-center gap-3 px-3 py-2 bg-surface border border-border rounded-lg">
                <div className="min-w-0 flex-1">
                  <p className="text-sm text-foreground font-medium truncate">
                    {s.name}
                    {s.builtIn && <span className="ml-2 px-1.5 py-0.5 text-[10px] rounded bg-accent-soft text-accent font-medium">default</span>}
                  </p>
                  <p className="text-[11px] text-muted truncate">{s.title}</p>
                </div>
                {s.hasSignature && s.signatureUrl ? (
                  /* eslint-disable-next-line @next/next/no-img-element */
                  <img src={s.signatureUrl} alt={`${s.name} signature`} className="h-8 max-w-28 object-contain bg-surface border border-border rounded px-1" />
                ) : (
                  <span className="text-[11px] text-subtle italic">no signature</span>
                )}
                <label className="flex items-center gap-1 px-2 py-1 text-[11px] font-medium rounded-md bg-accent-soft text-accent hover:bg-accent-soft cursor-pointer transition-colors shrink-0">
                ...
                <button onClick={() => void remove(s)} title={s.builtIn ? "Reset" : "Remove"}
                  className="p-1 rounded text-subtle hover:text-crit shrink-0">
          ...
          <div className="flex items-center gap-2 flex-wrap pt-1">
            ...
            <label className={`flex items-center gap-1 px-2 py-1.5 text-[11px] font-medium rounded-md border cursor-pointer transition-colors ${newFile ? "bg-ok/10 text-ok border-ok/20" : "bg-surface text-muted border-border hover:bg-surface-sunken"}`}>
            ...
            <button
              ...
              disabled={busyId === "new"}
              className="flex items-center gap-1 px-2.5 py-1.5 text-[11px] font-semibold rounded-md bg-accent text-accent-foreground hover:bg-accent-hover disabled:opacity-50 transition-colors">
...
function PersonExtras({ userId }: { userId: string }) {
  ...
  if (loading) return <div className="mt-4 pt-4 border-t border-border flex justify-center"><Loader2 className="w-5 h-5 animate-spin text-accent" /></div>;
    ...
    <div className="mt-4 pt-4 border-t border-border grid grid-cols-1 lg:grid-cols-2 gap-5">
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-semibold text-foreground flex items-center gap-1.5"><FileText className="w-3.5 h-3.5 text-accent" /> Documents</p>
          <div className="flex items-center gap-2">
            ...
            <label className="flex items-center gap-1 px-2 py-1 text-[11px] font-medium rounded-md bg-accent-soft text-accent hover:bg-accent-soft cursor-pointer transition-colors">
        ...
        {docs.length === 0 ? (
          <p className="text-xs text-muted py-3">No documents on file.</p>
        ) : (
          docs.map(d => (
            <div key={d.id} className="flex items-center justify-between gap-2 px-2 py-1.5 rounded-md hover:bg-surface-sunken">
              <div className="min-w-0">
                <p className="text-xs text-foreground truncate">{d.title}</p>
                <p className="text-[10px] text-muted">{d.category.replace("_", " ").toLowerCase()} · {fmtSize(d.size)} · {fmt(d.createdAt)}</p>
              </div>
              <div className="flex items-center gap-1 shrink-0">
                <a href={`/api/hr/documents/${d.id}`} target="_blank" rel="noreferrer" className="p-1 rounded text-muted hover:text-accent"><Download className="w-3.5 h-3.5" /></a>
                <button onClick={() => void removeDoc(d.id)} className="p-1 rounded text-muted hover:text-crit"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))
        )}
      ...
      <div>
        <div className="flex items-center justify-between mb-2">
          <p className="text-xs font-semibold text-foreground flex items-center gap-1.5"><ClipboardList className="w-3.5 h-3.5 text-accent" /> Onboarding checklist</p>
          {items.length === 0 && (
            <button onClick={() => void applyTemplate()}
              className="px-2 py-1 text-[11px] font-medium rounded-md bg-accent-soft text-accent hover:bg-accent-soft transition-colors">
              Apply default
            </button>
          )}
        </div>
        {items.map(item => (
          <div key={item.id} className="group flex items-center gap-2 px-2 py-1.5 rounded-md hover:bg-surface-sunken">
            <button onClick={() => void toggle(item)} className="shrink-0">
              {item.completedAt
                ? <Check className="w-3.5 h-3.5 text-ok" />
                : <div className="w-3.5 h-3.5 rounded-full border-2 border-border-strong" />}
            </button>
            <span className={`text-xs flex-1 ${item.completedAt ? "text-subtle line-through" : "text-foreground"}`}>{item.title}</span>
            <button onClick={() => void removeItem(item.id)} className="opacity-0 group-hover:opacity-100 p-0.5 text-subtle hover:text-crit"><X className="w-3 h-3" /></button>
          </div>
        ))}
        <div className="flex gap-2 mt-2">
          ...
          <button onClick={() => void addItem()} className="px-2.5 rounded-lg bg-accent-soft text-accent hover:bg-accent-soft"><Plus className="w-3.5 h-3.5" /></button>
        </div>
      </div>
    </div>
  );
...
function LeaveTab() {
  ...
  if (loading) return <div className="flex justify-center py-16"><Loader2 className="w-6 h-6 animate-spin text-accent" /></div>;

  const pending = requests.filter(r => r.status === "PENDING");
  const history = requests.filter(r => r.status !== "PENDING");

  const Row = ({ r, actions }: { r: LeaveRow; actions?: boolean }) => (
    <div className="flex items-center justify-between gap-3 px-3 py-2.5 bg-surface border border-border rounded-lg">
      <div className="flex items-center gap-3 min-w-0">
        <AvatarChip name={r.user.fullName} avatarUrl={r.user.avatarUrl} />
        <div className="min-w-0">
          <p className="text-sm text-foreground truncate">
            {r.user.fullName} <span className="text-muted">· {r.type.toLowerCase()}</span>
          </p>
          <p className="text-xs font-mono text-subtle">{fmt(r.startDate)} → {fmt(r.endDate)} · {r.days}d{r.reason ? ` · ${r.reason}` : ""}</p>
        </div>
      </div>
        ...
        {actions ? (
          <>
            <button onClick={() => void review(r.id, "approve")} disabled={actingId === r.id}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-ok/10 text-ok border border-ok/20 hover:bg-ok/20 disabled:opacity-50 transition-colors">
              <Check className="w-3.5 h-3.5" /> Approve
            </button>
            <button onClick={() => void review(r.id, "reject")} disabled={actingId === r.id}
              className="flex items-center gap-1 px-2.5 py-1.5 text-xs font-semibold rounded-lg bg-crit/10 text-crit border border-crit/20 hover:bg-crit/20 disabled:opacity-50 transition-colors">
  ...
  return (
    <div className="space-y-6">
      <div>
        <h3 className="text-sm font-semibold text-foreground mb-2">Pending approval · {pending.length}</h3>
        {pending.length === 0
  ...
  return (
    <div className="max-w-2xl space-y-4">
      <div className="flex gap-2">
        ...
        <button onClick={() => void add()} className="flex items-center gap-1.5 px-3 py-2 text-sm font-semibold rounded-lg bg-accent text-accent-foreground hover:bg-accent-hover shrink-0 transition-colors">
      ...
      {loading ? (
        ...
        <p className="text-sm text-muted py-6 text-center">No holidays for this year yet. Holidays are excluded from leave-day counts.</p>
      ) : (
        <div className="space-y-1.5">
          {holidays.map(h => (
            <div key={h.id} className="group flex items-center justify-between px-3 py-2.5 bg-surface border border-border rounded-lg">
              <span className="text-sm text-foreground">{h.name}</span>
              <div className="flex items-center gap-3">
                <span className="text-xs font-mono text-muted">{fmt(h.date)}</span>
                <button onClick={() => void remove(h.id)} className="opacity-0 group-hover:opacity-100 p-1 text-subtle hover:text-crit"><Trash2 className="w-3.5 h-3.5" /></button>
              </div>
            </div>
          ))}
        </div>
...
function OrgChartTab() {
  ...
  if (rows.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center py-16 text-center">
        <div className="w-14 h-14 rounded-2xl bg-accent-soft flex items-center justify-center mb-4"><Network className="w-7 h-7 text-accent" /></div>
        <p className="font-semibold text-foreground">No org chart yet</p>
        <p className="text-sm text-muted mt-1 max-w-xs">Fill in the reporting manager field in the People tab to build the org chart.</p>
      </div>
    );
  }
  ...
  const Node = ({ person, depth }: { person: StaffRow; depth: number }) => (
    <div style={{ marginLeft: depth === 0 ? 0 : 24 }}>
      <div className={`flex items-center gap-3 px-3 py-2 rounded-lg border mb-1.5 ${depth === 0 ? "bg-accent-soft border-accent/20" : "bg-surface border-border"}`}>
        <AvatarChip name={person.fullName} avatarUrl={person.avatarUrl} />
        <div className="min-w-0">
          <p className="text-sm text-foreground truncate">{person.fullName}</p>
          <p className="text-[11px] text-muted truncate">{person.jobTitle || person.role}{person.department ? ` · ${person.department}` : ""}</p>
        </div>
        {byManager[person.fullName]?.length ? (
          <span className="ml-auto text-[10px] font-mono text-subtle shrink-0">{byManager[person.fullName].length} report{byManager[person.fullName].length !== 1 ? "s" : ""}</span>
        ) : null}
      </div>
  ...
  return (
    <div className="max-w-2xl">
      <p className="text-sm text-muted mb-4">Built from the Reporting manager field in People. Anyone without a manager appears at the top level.</p>
