...
function StatusBadge({ status }: { status: BackupStatus | VerificationStatus }) {
  ...
  if (status === "not_tested") {
    return (
      <span className="inline-flex items-center gap-1 px-2 py-0.5 text-[10px] font-semibold rounded-full bg-[#5c6b72]/15 text-[#5c6b72] border border-[#5c6b72]/25">
...
function SummaryCard({
  ...
  return (
    <div className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl p-4 flex items-start gap-3">
      ...
      <div className="min-w-0 flex-1">
        ...
        {sub && <p className="text-xs text-[#5c6b72] mt-0.5">{sub}</p>}
      </div>
    </div>
  );
}
      ...
      <div className="px-6 pb-10 max-w-5xl space-y-6">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-xs font-semibold rounded-lg bg-[#262939] text-[#bbc9cf] border border-[rgba(0,255,255,0.08)] hover:bg-[#2e3347] transition-colors disabled:opacity-50"
          >
            {testingRestore ? (
              <Loader2 className="w-3.5 h-3.5 animate-spin" />
            ) : (
          ...
          <button
            onClick={() => void load()}
            className="p-2 text-[#5c6b72] hover:text-[#bbc9cf] transition-colors"
            title="Refresh"
          >
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {loading && rows.length === 0 ? (
                  <tr>
                    <td colSpan={5} className="text-center text-[#5c6b72] py-10">
                  ...
                  rows.map((row) => (
                    ...
                    >
                      ...
                      <td className="px-4 py-3 text-xs text-[#5c6b72]">{row.type}</td>
                      <td className="px-4 py-3">
                        <StatusBadge status={row.status} />
                      </td>
                      <td className="px-4 py-3 text-xs text-[#5c6b72] hidden sm:table-cell">
        ...
        <div className="bg-[#1b1f2e] border border-[rgba(0,255,255,0.08)] rounded-xl p-5">
          ...
          <p className="mt-4 text-xs text-[#5c6b72] leading-relaxed">
