    ...
    <div className="flex items-center -space-x-2">
      {visible.map((m) => (
        <div
          key={m.id}
          title={m.fullName}
          className="w-7 h-7 rounded-full border-2 border-border flex items-center justify-center text-[10px] font-semibold text-white"
      ...
      {overflow > 0 && (
        <div className="w-7 h-7 rounded-full border-2 border-border bg-surface-sunken flex items-center justify-center text-[10px] font-medium text-muted">
          +{overflow}
        </div>
      )}
    </div>
  ...
  return (
    <div className="border-t border-border bg-surface-sunken px-5 py-4">
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* Current members */}
        <div>
          <p className="text-[11px] text-subtle mb-2 font-semibold">
            Current Members ({currentMembers.length})
          </p>
          {currentMembers.length === 0 ? (
            <p className="text-xs text-subtle italic">No members yet.</p>
          ) : (
              ...
              {currentMembers.map((m) => (
                <li key={m.id} className="flex items-center gap-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    <p className="text-xs text-foreground truncate">{m.fullName}</p>
                    <p className="text-[10px] text-subtle truncate">{m.email}</p>
                  </div>
                  <button
                    onClick={() => toggle(m.id)}
                    title="Remove member"
                    className="p-1 text-subtle hover:text-crit transition-colors"
                  >
        ...
        <div>
          ...
          <div className="relative mb-2">
            ...
            <input
              ...
              className="w-full pl-8 pr-3 py-1.5 text-xs bg-surface-sunken border border-border-strong rounded-lg text-foreground placeholder-subtle focus:outline-none focus:border-accent/40 transition-colors"
            />
          </div>
          <ul className="space-y-1 max-h-44 overflow-y-auto pr-1">
            ...
            {filteredAll.filter((m) => !currentIds.has(m.id)).length === 0 && (
              <li className="text-xs text-subtle italic py-1">
      ...
      <div className="flex items-center justify-end gap-2 mt-4 pt-3 border-t border-border">
        <button
          onClick={save}
          disabled={saving}
          className="flex items-center gap-1.5 px-4 py-2 text-xs font-semibold rounded-lg bg-accent text-accent-foreground hover:bg-accent-hover transition-colors disabled:opacity-50"
        >
...
function CreateModal({
  ...
  return (
    ...
    >
      <div
        className="bg-surface border border-border rounded-xl shadow-2xl w-full max-w-lg flex flex-col max-h-[90vh]"
        onClick={(e) => e.stopPropagation()}
      >
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-border">
          <Mail className="w-4 h-4 text-accent flex-shrink-0" />
          <p className="flex-1 text-sm font-semibold text-foreground">New Shared Mailbox</p>
          <button onClick={onClose} className="p-1 text-subtle hover:text-muted">
            <X className="w-4 h-4" />
          </button>
        </div>
        ...
        <div className="overflow-y-auto flex-1 px-5 py-4 space-y-4">
          {/* Email */}
          <div>
            <label className="block text-[11px] text-subtle mb-1.5 font-semibold">
              Email address
            </label>
            <input
              type="email"
              ...
              className="w-full px-3 py-2 text-sm bg-surface border border-border rounded-lg text-foreground placeholder-subtle font-mono focus:outline-none focus:border-accent/40"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 text-sm bg-surface border border-border rounded-lg text-foreground placeholder-subtle focus:outline-none focus:border-accent/40"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              rows={2}
              className="w-full px-3 py-2 text-sm bg-surface border border-border rounded-lg text-foreground placeholder-subtle focus:outline-none focus:border-accent/40 resize-none"
            />
          </div>
          ...
          <div>
            ...
            <div className="relative mb-2">
              ...
              <input
                ...
                className="w-full pl-8 pr-3 py-1.5 text-xs bg-surface border border-border rounded-lg text-foreground placeholder-subtle focus:outline-none focus:border-accent/40"
              />
            </div>
            <ul className="space-y-1 max-h-44 overflow-y-auto border border-border rounded-lg p-1.5 bg-surface">
              {filtered.map((m) => {
                ...
                return (
                  <li
                    ...
                    className={`flex items-center gap-2.5 px-2 py-1.5 rounded-lg cursor-pointer transition-colors ${selected ? "bg-accent/10" : "hover:bg-surface"}`}
                  >
                    <div
                      className="w-6 h-6 rounded-full flex-shrink-0 flex items-center justify-center text-[9px] font-semibold text-white"
                      style={{ background: avatarGradient(m.email) }}
              ...
              {filtered.length === 0 && (
                <li className="text-xs text-subtle italic py-2 text-center">No users found.</li>
              )}
            </ul>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 px-5 py-3 border-t border-border">
          ...
          <button
            onClick={onClose}
            className="px-4 py-2 text-xs text-subtle hover:text-muted transition-colors"
          >
            Cancel
          </button>
        </div>
  ...
  return (
    <div className="bg-surface border border-border rounded-xl overflow-hidden">
      ...
      <div className="px-5 py-4 flex items-center gap-4">
        ...
        {/* Info */}
        <div className="flex-1 min-w-0">
          <p className="text-sm font-mono font-semibold text-foreground truncate">{mailbox.email}</p>
          <p className="text-xs text-subtle truncate mt-0.5">{mailbox.displayName}</p>
        </div>
        ...
        <div className="hidden sm:flex items-center gap-3">
          {members.length > 0 ? (
            ...
            <span className="text-xs text-subtle">No members</span>
          )}
        ...
        <div className="flex items-center gap-1.5 flex-shrink-0">
          <button
            ...
            className="flex items-center gap-1.5 px-3 py-1.5 text-xs font-semibold rounded-lg bg-surface-sunken text-muted hover:text-foreground hover:bg-hover transition-colors border border-border"
          >
          ...
          <button
            ...
            className="p-2 text-subtle hover:text-crit hover:bg-crit/10 rounded-lg transition-colors disabled:opacity-40"
          >
  ...
  return (
    <div className="min-h-full bg-surface text-foreground">
      <PageHeader
        ...
        action={
          <button
            ...
            className="flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-accent/10 text-accent border border-accent/20 hover:bg-accent/20 transition-colors"
          >
      ...
      <div className="px-6 pb-10 max-w-5xl">
        ...
        {/* Content */}
        {loading ? (
          <div className="flex items-center justify-center py-24 gap-3 text-subtle">
          ...
          <div className="flex flex-col items-center justify-center py-24 gap-4 text-center">
            ...
            <div>
              <p className="text-muted font-medium">No shared mailboxes yet.</p>
              <p className="text-sm text-subtle mt-1 max-w-sm">
                Create one for <span className="font-mono text-muted">support@</span>,{" "}
                <span className="font-mono text-muted">sales@</span>, or{" "}
                <span className="font-mono text-muted">billing@</span> to let your team
                collaborate on a shared inbox.
              </p>
            </div>
            <button
              onClick={() => setShowCreate(true)}
              className="flex items-center gap-2 px-5 py-2.5 text-sm font-semibold rounded-lg bg-accent/10 text-accent border border-accent/20 hover:bg-accent/20 transition-colors mt-1"
            >
              <Plus className="w-4 h-4" />
          ...
          <div className="bg-surface border border-border rounded-xl overflow-hidden">
            <div className="flex items-center gap-3 px-5 py-3 border-b border-border">
              ...
              <span className="ml-auto text-xs text-subtle">{mailboxes.length} total</span>
            </div>
                ...
                <tbody>
                  {mailboxes.map((mb) => {
                    ...
                    return (
                      ...
                      >
                        <td className="px-5 py-3 font-mono text-xs text-foreground">{mb.email}</td>
                        <td className="px-5 py-3 text-xs text-muted hidden sm:table-cell">{mb.displayName}</td>
                        ...
                        <td className="px-5 py-3 text-right">
                          <div className="flex items-center justify-end gap-1.5">
                            <button
                              ...
                              className="px-2.5 py-1 text-[10px] font-semibold rounded bg-surface-sunken text-muted hover:text-foreground border border-border transition-colors"
                            >
                              Members
                            </button>
                            <button
